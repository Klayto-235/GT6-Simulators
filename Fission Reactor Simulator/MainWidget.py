...
class DuctTape(QWidget):
    ...
    def on_close_event(self, event):
        ...
        if not is_max:
            Settings().setList([("PosX", pos_data.x()), ("PosY", pos_data.y()), ("SizeX", size_data.width()),
                                ("SizeY", size_data.height())])
        event.accept()
    ...
    def invalidate_save_state(self, mark_in_history):
        if self.save_state:
            ...
            self.update_main_window_title()
            if mark_in_history:
                self.historian.commit_event((self.revalidate_save_state, (self.save_count,)),
                                            (self.invalidate_save_state, ()))
    ...
    def simulate(self, autorun=False):
        ...
        self.historian.insert_commit((self.invalidate_simulation_state, ()),
                                     (self.revalidate_simulation_state, (self.sim_count,)))
    ...
    def invalidate_simulation_state(self, mark_in_history):
        ...
        if self.sim_state:
            ...
            if mark_in_history:
                self.historian.commit_event((self.revalidate_simulation_state, (self.sim_count,)),
                                            (self.invalidate_simulation_state, ()))
