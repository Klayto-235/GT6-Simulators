...
class GeneralOptionsTab(QScrollArea):
    def __init__(self, parent=None):
        ...
        self.layout.addWidget(self.klayout, tcount, 1)
        ...
        templabel.setToolTip(
            "Time limit (in seconds) for simulations that run automatically.\nAccepts values between 0.1 and 60.0 with up to microsecond precision.")
        self.layout.addWidget(templabel, tcount, 0)
        self.artimelimit = QLineEdit(Settings().get("AutosimTimeout"), self.inside)
        self.artimelimit.setValidator(validator)
        self.artimelimit.setToolTip(
            "Time limit (in seconds) for simulations that run automatically.\nAccepts values between 0.1 and 60.0 with up to microsecond precision.")
        self.layout.addWidget(self.artimelimit, tcount, 1)

        tcount += 1

        templabel = QLabel("Separate stacks in material cost: ", self.inside)
        templabel.setToolTip("When enabled will break down the raw material costs into stacks of 64 + remainder.")
        self.layout.addWidget(templabel, tcount, 0)
        ...
        self.layout.addWidget(self.initw, tcount, 1)
        ...
        self.layout.addWidget(self.inith, tcount, 1)
        ...
        self.layout.addWidget(self.minz, tcount, 1)
        ...
        self.layout.addWidget(self.maxz, tcount, 1)

        tcount += 1

        templabel = QLabel("Graphics engine: ", self.inside)
        templabel.setToolTip(
            "Choose preferred graphics engine, the default is a CPU raster engine.\nREQUIRES RESTART TO TAKE EFFECT.")
        self.layout.addWidget(templabel, tcount, 0)
        ...
        self.layout.addWidget(self.gen, tcount, 1)
    ...
    def apply_options(self):
        ...
        test = False
        test = test or (Settings().get_bool_delayed("Antialiasing") != Settings().get_bool(
            "Antialiasing") and tempAA != Settings().get_bool_delayed("Antialiasing"))
        test = test or (Settings().get_delayed("GraphicsEngine") != Settings().get(
...
class OptionsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.button_box_widget = QDialogButtonBox(
            QDialogButtonBox.StandardButton.Apply | QDialogButtonBox.StandardButton.Save | QDialogButtonBox.StandardButton.Cancel | QDialogButtonBox.StandardButton.RestoreDefaults)
        ...
        self.tab_widget.setTabToolTip(1,
                                      "List of keyboard shortcuts (you can't edit these directly... because reasons)")
