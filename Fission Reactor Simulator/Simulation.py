...
SimulationData = namedtuple("SimulationData",
                            ["stopped_reason", "total_HU", "total_max_N", "peak_HUt", "exploded", "duration",
                             "current_time", "current_HUt", "coolant_use"])
RodSelectionData = namedtuple("RodSelectionData",
                              ["rod_total_HU", "rod_peak_N", "rod_penalty", "rod_moderated", "current_durability",
                               "current_N_count", "current_N_output", "ref_mult", "rod_total_HUby", "rod_total_N",
                               "HUt_by_rod"])
CellSelectionData = namedtuple("CellSelectionData",
...
class Rod():
    ...
    def set(self, whandle, rod_id, coolant_id, csf):
        ...
        self.neutron_emission = divup(
            Assets().rod[rod_id].neutron_emission * Assets().coolant[coolant_id].mod_emission[0],
            Assets().coolant[coolant_id].mod_emission[1])
        self.neutron_max = divup(Assets().rod[rod_id].neutron_max * Assets().coolant[coolant_id].mod_max[0],
                                 Assets().coolant[coolant_id].mod_max[1])
        self.neutron_div = Assets().coolant[coolant_id].mod_div[0](Assets().rod[rod_id].neutron_div,
    ...
    def post_rod_data(self, index):
        if self.is_fuel:
            self.widget_handle.set_durability(
                self.results.durability_data[index][1] / self.max_durability / 120000 * 100.0)
        elif self.is_breeder:
            self.widget_handle.set_durability(
                100.0 - self.results.durability_data[index][1] / self.max_durability * 100.0)
...
class Cell():
    ...
    def post_cell_data(self, index):
        self.widget_handle.set_coolant_flux("{:.2f}".format(self.results.Lt_data[index][1]),
                                            str(self.results.HUt_data[index][1]), self.exploded)
...
class Simulation():
    ...
    def simulate(self, autorun):
        ...
        while (self.simulated_time < self.max_time and self.exploded == False and self.has_depleted_rod == False and (
                abs(self.prev_HUt - self.HUt) > 0 or self.moderation_changed == True)):
                ...
                if self.rod_data[j].is_fuel or self.rod_data[j].is_abs:
                        ...
                        if self.rod_data[j].cell_size_small == False:
                            ...
                            self.rod_data[i].HUtby += divup(
                                2 * divup(self.rod_data[i].real_neutron_emission, 2) * self.rod_data[
                                    j].HU_conversion_factor, self.rod_data[j].HU_conversion_divisor)
                        else:
                            self.rod_data[j].neutron_count += divup(self.rod_data[i].real_neutron_emission, 2)
                            self.rod_data[i].HUtby += divup(
                                divup(self.rod_data[i].real_neutron_emission, 2) * self.rod_data[
                                    j].HU_conversion_factor, self.rod_data[j].HU_conversion_divisor)
                    else:
                    ...
                    if self.rod_data[i].cell_size_small == False:
                        if self.rod_data[j].cell_size_small == False:
                            ...
                            self.rod_data[i].HUtby += divup(
                                2 * divup(self.rod_data[i].real_neutron_emission * self.rod_data[j].mod_factor, 2) *
                                self.rod_data[i].HU_conversion_factor, self.rod_data[i].HU_conversion_divisor)
                        else:
                            self.rod_data[i].neutron_count += divup(
                                self.rod_data[i].real_neutron_emission * self.rod_data[j].mod_factor, 2)
                            self.rod_data[i].HUtby += divup(
                                divup(self.rod_data[i].real_neutron_emission * self.rod_data[j].mod_factor, 2) *
                                self.rod_data[i].HU_conversion_factor, self.rod_data[i].HU_conversion_divisor)
                    else:
                        self.rod_data[i].neutron_count += self.rod_data[i].real_neutron_emission * self.rod_data[
                            j].mod_factor
                        self.rod_data[i].HUtby += divup(
    ...
    def collect_coolant_data(self):
        for i in range(len(self.cell_data)):
            self.results.coolant_use_data[self.cell_data[i].coolant_name] = self.results.coolant_use_data.get(
                self.cell_data[i].coolant_name, 0) + self.cell_data[i].results.Lt_data[-1][1]

    def post_plot_data(self):
        self.info_panel.set_plot_data(self.results.HUt_data)

    def post_sim_data(self, index):
        self.info_panel.set_simulation_data(
            SimulationData(self.reason, self.results.totalHU, self.max_N_count, self.results.HUt_data[-1][1],
                           self.exploded, self.simulated_time, self.results.HUt_data[index][0],
                           self.results.HUt_data[index][1], self.results.coolant_use_data))
        for i in range(len(self.rod_data)):
    ...
    def post_selection_data(self, index, target):
        ...
        if rod_id is not None:
            ...
            if trod.is_fuel:
                tempstr = "{:.3f}".format(trod.results.durability_data[index][1] / 120000) + "/" + str(
                    Assets().rod[trod.rod_name].durability)
            elif trod.is_breeder:
                tempstr = str(trod.results.durability_data[index][1]) + "/" + str(
                    Assets().rod[trod.rod_name].durability)
            else:
                tempstr = "infinite"
            if trod.is_fuel:
                ...
                tstroutput = "0"
            rdata = RodSelectionData(trod.results.totalHU, trod.results.max_output, trod.over_neutron_max,
                                     trod.is_moderated, tempstr, trod.results.neutron_count_data[index][1], tstroutput,
                                     trod.mod_factor, trod.results.totalHUby, trod.results.totalN,
                                     trod.results.HUtby_data[index][1])
            cdata = CellSelectionData(tcell.results.totalL, tcell.results.Lt_data[-1][1], tcell.exploded,
                                      tcell.coolant_name == "H2O", tcell.results.HUt_data[index][1],
                                      tcell.results.Lt_data[index][1])
