...
def to_whole_materials(input):
	...
	if input % 72 == 0:
		...
		if (temp1 == 0):
			...
			return str(temp1) + "+" + str(temp2 // temp3) + "/" + str(72 // temp3)


def recursive_material_cost(key, value, raw):
		if key in Assets().material_costs:
			for k,v in Assets().material_costs[key].items():
				value[1][k] = [v * value[0], {}]
				recursive_material_cost(k, value[1][k], raw)
				value[1][k][0] = str(value[1][k][0])
		else:
			for k,v in Assets().raw_materials[key].items():
				value[1][k] = [to_whole_materials(v * value[0]), {}]
				if k in raw:
					raw[k][0] += v * value[0]
					if key in raw[k][1]:
